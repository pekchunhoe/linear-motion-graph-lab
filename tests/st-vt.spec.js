...
async function geometry(page) {
  return page.evaluate(()=>{
    ...
    return {overflow:document.documentElement.scrollWidth>innerWidth,car:rect('#sim1-carContainer'),road:rect('.road'),
      canvases:[...document.getElementById('sim1').querySelectorAll('canvas:not(.profile-preview)')].map(c=>({width:c.clientWidth,height:c.clientHeight,backing:c.width,dpr:devicePixelRatio,focus:c.tabIndex,label:c.getAttribute('aria-label')})),
